...
./create-user-key.ps1 -Budget 50          # $50 gpt-4.1 virtual key (persists in Postgres)
```
...
- **Model/router config** (the 2 Foundry backends + load balancing) lives in the **config file**, not the DB. It's reproducible from this module.
- **PostgreSQL** stores only **virtual keys + spend/budgets**. These survive **app restarts** and revision changes.
- Restarting the app reloads config from the file; restarting the DB keeps the keys.
