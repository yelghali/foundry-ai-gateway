...
└── src/
    ...
    │   ├── agent_foundry_native.py  # Part 5: native Foundry agent — model + MCP + A2A, no gateway│   │   ├── agent_foundry_client.py  # Client: one "consumer" Foundry reaching enterprise models (3 ways) + MCP (APIM/LiteLLM) + A2A    │   └── requirements.txt
    └── litellm/
...
> - **Enterprise-vs-client topology (the "recreated lab")** — `client-foundry.bicep` stands up a **separate "consumer" Foundry** (`foundry-client`, project `aigateway-client`) that hosts **no enterprise models of its own** and reaches everything through connections. `agent_foundry_client.py` runs one consumer agent per path and reports PASS/FAIL. Validated live: **models 2 ways** — `apim-gateway/gpt-4o-mini` (ApiManagement = AI Gateway native) and `litellm-gateway/gpt-4o-mini` (ModelGateway = BYO) both **PASS**; **MCP 2 ways** — MS Learn behind **APIM** (`{apim}/learn-mcp/mcp`) and behind **LiteLLM** (`{litellm}/mcp/`), both **CustomKeys** connections, both **PASS**; **A2A** — the remote specialist via a **`RemoteA2A`** connection to the agent's **host root**, **PASS**. Two honest findings: **(1)** a **`CustomKeys` connection cannot back a *model*** (`apim-custom/gpt-4o-mini` → `400 Category cannot be null` — Foundry serves models only via `ApiManagement`/`ModelGateway`; custom keys are for **tool** auth); **(2)** the **managed A2A tool 500s when the calling agent's model is a gateway connection** — it must be driven by a **native deployment**, so the client account hosts one small `gpt-4o-mini` *driver* deployment (plain model calls and MCP work fine over the gateway connections; only A2A needs the local driver).
