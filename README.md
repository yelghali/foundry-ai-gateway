...
│   ├── client-foundry.bicep         # Client: a separate "consumer" Foundry + 6 connections (no enterprise models of its own)
│   ├── deploy-client-foundry.ps1    # Client: deploy the consumer Foundry on top of main + LiteLLM + A2A
...
└── src/
    ...
    │   ├── scenario_lib.py          # Part 6: shared helpers for the 3 client scenarios
    │   ├── scenario1_custom_apim.py # Part 6 — Scenario 1 (custom/APIM): model + tool + A2A
    │   ├── scenario2_aigateway_native.py   # Part 6 — Scenario 2 (AI Gateway native): model + tool + A2A
...
> - **Enterprise-vs-client topology — 3 consumer scenarios (Part 6)** — `client-foundry.bicep` stands up a **separate "consumer" Foundry** (`foundry-client`, project `aigateway-client`) that hosts **no enterprise models of its own** and reaches the same three remote targets (an enterprise **model**, **MS Learn MCP**, a **remote A2A specialist**) three ways. Each scenario runs **model → tool → A2A** and reports PASS/FAIL. Validated live (**8 PASS / 1 expected fail**): **Scenario 1 — custom (APIM)** [`scenario1_custom_apim.py`]: model ⛔ *expected fail* (a `CustomKeys` connection can't back a model — `400 Category cannot be null`), tool ✅ (MS Learn MCP behind APIM), A2A ✅; **Scenario 2 — AI Gateway native** [`scenario2_aigateway_native.py`]: model ✅ (`apim-gateway/gpt-4o-mini`, `ApiManagement`), tool ✅ (same APIM gateway), A2A ✅; **Scenario 3 — AI Gateway LiteLLM** [`scenario3_aigateway_litellm.py`]: model ✅ (`litellm-gateway/gpt-4o-mini`, `ModelGateway`), tool ✅ (MS Learn MCP behind LiteLLM), A2A ✅. Two honest findings: **(1)** a **`CustomKeys` connection cannot back a *model*** (custom keys are for **tool** auth instead); **(2)** the **managed A2A tool 500s when the calling agent's model is a gateway connection**, so every A2A leg is driven by **one small native `gpt-4o-mini` driver** the client account hosts (plain model calls and MCP work fine over gateway connections; only A2A needs the native driver — and A2A through a path-scoped gateway is blocked by Foundry's host-root card discovery, so all scenarios reach the specialist directly).
