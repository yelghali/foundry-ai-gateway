...
Every surface is tested by two consumers: a local Microsoft Agent Framework application and a prompt agent hosted by Foundry Agent Service.
...
This workshop uses a Foundry **bring-your-own-model `ApiManagement` connection** to customer-owned APIM. It does not use Foundry's integrated AI Gateway attachment, LiteLLM, or a `ModelGateway` connection.
...
`litellm-gateway/`, `rag-demo/`, and older scenario/deployment files are separate experiments. They are not part of the keyless workshop path above.
